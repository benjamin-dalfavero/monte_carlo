...
% define bins
% make nbins number of linearly spaced bins in concentrated region
% planck function is concentrated from 0 < lT < 4e4
lambda_min = 0;
lambda_max = 4e4 / T;
edges_conc = linspace(lambda_min, lambda_max, nbins);
...
% divide energy by wavelengths to get emissive power
power = energy ./ lam_mid;
% interpolate exact solution for emissive power
IsT_exact = interp1(lt, IsT, edges * T);
I_exact = IsT_exact * sigma * T^5;
Eb_exact = I_exact * pi;
% percent error in approximation
err = abs(power - Eb_exact) ./ abs(Eb_exact);

%% write results to a table
res = [edges; counts; energy; power; Eb_exact; err]';
headings = {'wavelength', 'bundles', 'energy', 'power', 'Elb', 'error'};
